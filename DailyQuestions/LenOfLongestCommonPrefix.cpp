...
// **Q: Find the Length of the Longest Common Prefix **
...
public:
    int longestCommonPrefix(vector<int>& arr1, vector<int>& arr2) {
        unordered_map <int, int> prefix; // storing all possible prefix of arr1 in hashmap
        ...
        for (auto num : arr2) {
            ...
            while (i<=s.size()) {
                ...
                if (prefix.find(t) != prefix.end()) {
                    len = max(len,i); // checking size for each case
                }
                else break; // no need to continue loop in case of mismatch
                i++;
            }
        }
        return len;
